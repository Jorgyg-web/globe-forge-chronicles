...
import { BarChart3, Sword, Globe, FlaskConical, LayoutDashboard, MapPin, Hammer, Factory } from 'lucide-react';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';

const panels: { id: PanelType; label: string; icon: React.ReactNode }[] = [
  { id: 'overview', label: 'Overview', icon: <LayoutDashboard size={16} /> },
  { id: 'province', label: 'Provinces', icon: <MapPin size={16} /> },
          ...
          return (
            <Tooltip key={p.id}>
              <TooltipTrigger asChild>
                <button onClick={() => setActivePanel(p.id)}
                  className={`relative w-9 h-9 rounded-lg flex items-center justify-center transition-all group ${isActive ? 'bg-primary/15 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}>
                  {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-primary rounded-r-full" />}
                  {p.icon}
                </button>
              </TooltipTrigger>
              <TooltipContent side="right" className="text-xs font-mono">{p.label}</TooltipContent>
            </Tooltip>
          );
        })}
      </div>
