...
import { Globe, Handshake, ShieldAlert, Ban, ArrowRightLeft, Swords } from 'lucide-react';
            ...
            {state.alliances.filter(a => a.members.includes(state.playerCountryId)).map(a => (
              <div key={a.id} className="flex items-center justify-between">
                ...
                <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
                  a.type === 'military' ? 'bg-danger/10 text-danger' :
                  a.type === 'economic' ? 'bg-success/10 text-success' :
        ...
        <div className="divide-y divide-border/20">
          {relations.map(([otherId, rel]) => {
            ...
            return (
              <div
                key={otherId}
                className="p-2.5 hover:bg-muted/20 transition-colors cursor-pointer"
                onClick={() => setSelectedCountryId(otherId)}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded flex items-center justify-center text-[8px] font-mono font-bold" style={{ backgroundColor: other.color + '30', color: other.color }}>
                      {other.code}
                    </div>
                    <span className="text-xs font-medium text-foreground">{other.name}</span>
                    {atWar && (
                      <span className="text-[9px] px-1.5 py-0.5 bg-danger/15 text-danger rounded font-mono font-semibold border border-danger/20">WAR</span>
                    )}
                    {embargo && (
                      <span className="text-[9px] px-1.5 py-0.5 bg-warning/15 text-warning rounded font-mono font-semibold border border-warning/20">EMBARGO</span>
                    )}
                  </div>
                  <RelationBadge value={rel} />
                </div>
                ...
                <div className="w-full h-1 bg-muted rounded-full overflow-hidden mb-2 relative">
                  ...
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${rel > 0 ? 'bg-success' : 'bg-danger'}`}
                    style={{
                      width: `${Math.abs(rel) / 2}%`,
                      marginLeft: rel > 0 ? '50%' : `${50 - Math.abs(rel) / 2}%`,
                    }}
                  />
                </div>
                ...
                <div className="flex gap-1 flex-wrap">
                  ...
                  {!embargo ? (
                    ...
                    <ActionBtn icon={<ShieldAlert size={9} />} label="Lift Embargo" onClick={() => dispatch({ type: 'REMOVE_EMBARGO', countryId: state.playerCountryId, targetId: otherId })} />
                  )}
                </div>
              </div>
            );
...
const RelationBadge = ({ value }: { value: number }) => {
  const color = value > 50 ? 'text-stat-positive' : value > 0 ? 'text-foreground' : value > -50 ? 'text-stat-neutral' : 'text-stat-negative';
  return (
